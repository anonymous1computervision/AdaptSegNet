...
class AdaptSeg_Edge_Aux_Multi_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(AdaptSeg_Edge_Aux_Multi_Trainer, self).__init__()
        ...
        assert hyperparameters["model"] == 'DeepLabEdgeMulti', True
        ...
        self.model_D_final = Gated_Discriminator(num_classes=hyperparameters['num_classes'] + 1)
        ...
        self.loss_names = ['Seg', 'Seg_last', 'Edge_Seg', 'LAST_GAN_dis', 'LAST_GAN_adv', 'FINAL_GAN_dis', 'FINAL_GAN_adv']
    ...
    def gen_target_update(self, images, image_path):
        ...
        loss = self.lambda_adv_target*loss_adv + self.lambda_adv_target_coarse*loss_adv_last
        ...
        self.loss_dict['LAST_GAN_adv'] += loss_adv_last.data.cpu().numpy()
        self.loss_dict['FINAL_GAN_adv'] += loss_adv.data.cpu().numpy()
    ...
    def dis_update(self, labels=None):
        ...
        d_out_real_final, _ = self.model_D_final(F.softmax(self.source_image, dim=1), label=self.pred_real_edge)
        ...
        # d_out_fake, _ = self.model_D(net_input, label=self.pred_fake_edge)
        d_out_fake_final, _ = self.model_D_final(F.softmax(self.target_image, dim=1), label=self.pred_fake_edge)
        ...
        self.loss_dict['FINAL_GAN_dis'] += loss_final.data.cpu().numpy()
        self.loss_dict['LAST_GAN_dis'] += loss_last.data.cpu().numpy()
