...
class AdaptSeg_Edge_Aux_Triple_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(AdaptSeg_Edge_Aux_Triple_Trainer, self).__init__()
        ...
        assert hyperparameters["model"] == 'DeepLabEdgeTriple', True
        ...
        self.model_D_last = SP_FCDiscriminator(num_classes=hyperparameters['num_classes'])
        self.model_D_final = SP_FCDiscriminator(num_classes=hyperparameters['num_classes'])
        ...
        self.model_D_last.train()
        self.model_D_last.cuda(self.gpu)
        self.model_D_final.train()
        self.model_D_final.cuda(self.gpu)
        ...
        self.lambda_seg_last = hyperparameters['gen']['lambda_last_edge']
        self.lambda_adv_target = hyperparameters['gen']['lambda_adv_target']
        self.lambda_adv_target_coarse = hyperparameters['gen']['lambda_adv_target_coarse']
        ...
        self.loss_names = ['Seg', 'Seg_last', 'Edge_Seg', 'LAST_dis', 'LAST_adv', 'FINAL_dis', 'FINAL_adv', 'FORE_dis', 'FORE_adv']
    ...
    def init_opt(self):
        ...
        self.optimizer_D_last.zero_grad()
        self._adjust_learning_rate_D(self.optimizer_D_last, 0)

        self.optimizer_D_final = optim.Adam([p for p in self.model_D_final.parameters() if p.requires_grad],
                                                 lr=self.lr_d, betas=(self.D_beta1, self.D_beta2))
        self.optimizer_D_final.zero_grad()
        self._adjust_learning_rate_D(self.optimizer_D_final, 0)
    ...
    def forward(self, images):
        # self.eval()
        # pdb.set_trace()
        predict_seg, _, _ = self.model(images)
        # self.train()
        return predict_seg
    ...
    def gen_source_update(self, images, labels, label_path=None):
        ...
        for param in self.model_D_last.parameters():
            param.requires_grad = False

        for param in self.model_D_final.parameters():
            param.requires_grad = False
        ...
        # get predict output
        pred_source_real, pred_source_edge, pred_source_real_last = self.model(images)
        ...
        pred_source_real_last = interp(pred_source_real_last)
        ...
        seg_loss_last = self._compute_seg_loss(pred_source_real_last, labels)
        ...
        seg_loss = self.lambda_seg * seg_loss + self.lambda_seg_last * seg_loss_last + self.lambda_adv_edge * attn_loss
    ...
    def gen_target_update(self, images, image_path):
        ...
        # get predict output
        pred_target_fake, pred_target_edge, pred_target_fake_last = self.model(images)
        ...
        d_out_fake, _ = self.model_D_final(F.softmax(pred_target_fake, dim=1), label=None)
        d_out_fake_foreground, _ = self.model_D_foreground(F.softmax(pred_target_fake, dim=1), label=pred_target_edge)
        ...
        loss = self.lambda_adv_target*loss_adv_final + \
               self.lambda_adv_target*self.lambda_adv_foreground*loss_adv_foreground + \
               self.lambda_adv_target_coarse*loss_adv_last
        ...
        self.target_last_image = pred_target_fake_last.detach()
        ...
        self.loss_dict['LAST_adv'] += loss_adv_last.data.cpu().numpy()
        self.loss_dict['FINAL_adv'] += loss_adv_final.data.cpu().numpy()
        self.loss_dict['FORE_adv'] += loss_adv_foreground.data.cpu().numpy()
    ...
    def dis_update(self, labels=None):
        ...
        d_out_real_final, _ = self.model_D_final(F.softmax(self.source_image, dim=1), label=None)
        d_out_real_foreground, _ = self.model_D_foreground(F.softmax(self.source_image, dim=1), label=self.pred_real_edge)

        d_out_real_last, _ = self.model_D_last(F.softmax(self.source_last_image, dim=1), label=None)
        ...
        # d_out_fake, _ = self.model_D(net_input, label=self.pred_fake_edge)
        d_out_fake_final, _ = self.model_D_final(F.softmax(self.target_image, dim=1), label=None)
        d_out_fake_foreground, _ = self.model_D_foreground(F.softmax(self.target_image, dim=1), label=self.pred_fake_edge)

        d_out_fake_last, _ = self.model_D_last(F.softmax(self.target_last_image, dim=1), label=None)
        ...
        if self.adv_loss_opt == "hinge":
            ...
            loss_last = self.loss_hinge_dis(d_out_fake_last, d_out_real_last)
            loss_last.backward()
            loss_final = self.loss_hinge_dis(d_out_fake_final, d_out_real_final)
            loss_final.backward()
            loss_foreground = self.loss_hinge_dis(d_out_fake_foreground, d_out_real_foreground)
            loss_foreground.backward()
            ...
            loss_fake_last = self._compute_adv_loss_fake(d_out_fake_last, self.adv_loss_opt)
            loss_real_last = self._compute_adv_loss_real(d_out_real_last, self.adv_loss_opt)
            loss_last = (loss_real_last + loss_fake_last) / 2
            loss_last.backward()
            loss_fake_final = self._compute_adv_loss_fake(d_out_fake_final, self.adv_loss_opt)
            loss_real_final = self._compute_adv_loss_real(d_out_real_final, self.adv_loss_opt)
            loss_final = (loss_real_final + loss_fake_final) / 2
            loss_final.backward()
            loss_fake_final = self._compute_adv_loss_fake(d_out_fake_foreground, self.adv_loss_opt)
            loss_real_final = self._compute_adv_loss_real(d_out_real_foreground, self.adv_loss_opt)
            loss_final = (loss_real_final + loss_fake_final) / 2
            loss_final.backward()
        # update loss
        self.optimizer_D_last.step()
        self.optimizer_D_final.step()
        self.optimizer_D_foreground.step()

        # self.loss_d_foreground_value += loss.data.cpu().numpy()
        self.loss_dict['FINAL_dis'] += loss_final.data.cpu().numpy()
        self.loss_dict['LAST_dis'] += loss_last.data.cpu().numpy()
        self.loss_dict['FORE_dis'] += loss_foreground.data.cpu().numpy()
    ...
    def update_learning_rate(self):
        ...
        if self.optimizer_D_last:
            self.optimizer_D_last.zero_grad()
            self._adjust_learning_rate_D(self.optimizer_D_last, self.i_iter)

        if self.optimizer_D_final:
            self.optimizer_D_final.zero_grad()
            self._adjust_learning_rate_D(self.optimizer_D_final, self.i_iter)
    ...
    def snapshot_image_save(self, dir_name="check_output/", src_save=True, target_save=True):
        ...
        if not os.path.exists(os.path.join(dir_name, "Image_source_domain_last_seg")):
            os.makedirs(os.path.join(dir_name, "Image_source_domain_last_seg"))
        if not os.path.exists(os.path.join(dir_name, "Image_target_domain_last_seg")):
            os.makedirs(os.path.join(dir_name, "Image_target_domain_last_seg"))
        ...
        if src_save:
            ...
            paint_predict_image(self.source_last_image).save('check_output/Image_source_domain_last_seg/%s.png' % self.i_iter)
        ...
        if target_save:
            ...
            paint_predict_image(self.target_last_image).save('check_output/Image_target_domain_last_seg/%s.png' % self.i_iter)
    ...
    def save_model(self, snapshot_save_dir="./model_save"):
        ...
        torch.save(self.model_D_last.state_dict(), os.path.join(snapshot_save_dir, 'GTA5_' + str(self.i_iter) + '_D_last.pth'))
        torch.save(self.model_D_final.state_dict(),
                   os.path.join(snapshot_save_dir, 'GTA5_' + str(self.i_iter) + '_D_final.pth'))
