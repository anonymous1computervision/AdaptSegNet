...
class Xception(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, low_level_feat, middel_out
...
class Classifier_Module_v3(nn.Module):
    def __init__(self, inplanes=2048, dilation_series=[1, 6, 12, 18], n_classes=19):
        super(Classifier_Module_v3, self).__init__()
        # ASPP
        rates = dilation_series
        self.aspp1 = ASPP_module(inplanes, 256, rate=rates[0])
        self.aspp2 = ASPP_module(inplanes, 256, rate=rates[1])
        self.aspp3 = ASPP_module(inplanes, 256, rate=rates[2])
        self.aspp4 = ASPP_module(inplanes, 256, rate=rates[3])
...
class DeepLabv3_plus_edge(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, pretrained=False, _print=True):
        ...
        self.pred_edge = Classifier_Module_v3(inplanes=728, n_classes=1)
        self.pred = Classifier_Module_v3(inplanes=2048, n_classes=n_classes)


    def forward(self, x):
        x, low_level_features, middle_out = self.xception_features(x)

        # print("x shape =", x.shape)
        # print("low_level_features shape =", low_level_features.shape)
        # print("middle_out shape =", middle_out.shape)

        x_edge = self.pred_edge(middle_out, low_level_features)
        # print("x_edge out  shape =", x_edge.shape)

        x = self.pred(x, low_level_features)
