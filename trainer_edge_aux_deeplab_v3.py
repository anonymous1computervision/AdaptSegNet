...
class AdaptSeg_Deeplabv3_Edge_Aux_Trainer(nn.Module):
	def __init__(self, hyperparameters):
		super(AdaptSeg_Deeplabv3_Edge_Aux_Trainer, self).__init__()
		...
		print("AdaptSeg_Deeplabv3_Edge_Aux_Trainer")
		# init G
        #
	...
	def gen_target_update(self, images, image_path):
		...
		self.loss_dict['Global_GAN_adv'] += adv_loss.data.cpu().numpy()
	...
	def _adjust_learning_rate_G(self, optimizer, i_iter):
		...
		# print("self.scheduler_G.get_lr()=", self.scheduler_G.get_lr()[0])
			lr = self._lr_poly(self.scheduler_G.get_lr()[0], i_iter, self.num_steps, self.decay_power)
		else:
