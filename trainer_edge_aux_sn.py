...
from model.deeplab_single_add_edge_act_match import Res_Deeplab as Res_Deeplab_Edge_SN
...
class AdaptSeg_Edge_Aux_SN_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(AdaptSeg_Edge_Aux_SN_Trainer, self).__init__()
        ...
        elif hyperparameters["model"] == 'DeepLabEdgeSN':
            self.model = Res_Deeplab_Edge_SN(num_classes=hyperparameters["num_classes"])
            print("use Res_Deeplab_Edge_SN model")
        ...
        self.model_D_match_l2 = SP_FCDiscriminator(num_classes=1024)
        self.model_D_match_l1 = SP_FCDiscriminator(num_classes=2048)
        ...
        self.model_D_match_l1.train()
        self.model_D_match_l1.cuda(self.gpu)
        self.model_D_match_l2.train()
        self.model_D_match_l2.cuda(self.gpu)
        ...
        self.loss_g_match_l1_value = 0
        self.loss_g_match_l2_value = 0
        self.loss_d_match_l1_value = 0
        self.loss_d_match_l2_value = 0
    ...
    # add match activation D

        self.optimizer_D_l2 = optim.Adam([p for p in self.model_D_match_l2.parameters() if p.requires_grad],
                                                 lr=self.lr_d, betas=(self.beta1, self.beta2))
        self.optimizer_D_l2.zero_grad()
        self._adjust_learning_rate_D(self.optimizer_D_l2, 0)

        self.optimizer_D_l1 = optim.Adam([p for p in self.model_D_match_l1.parameters() if p.requires_grad],
                                                 lr=self.lr_d, betas=(self.beta1, self.beta2))
        self.optimizer_D_l1.zero_grad()
        self._adjust_learning_rate_D(self.optimizer_D_l1, 0)
    ...
    def gen_source_update(self, images, labels, label_path=None):
        ...
        # get predict output
        pred_source_real, pred_source_edge, l2, l1 = self.model(images)
        self.source_l2 = l2.detach()
        self.source_l1 = l1.detach()
    ...
    def gen_target_update(self, images, image_path):
        ...
        loss = 0.1 * self.lambda_adv_target * adv_loss
        loss.backward()
        ...
        self.gen_target_update_match(images, image_path)
        self.match_dis_update()

    def gen_target_update_match(self, images, image_path):

        # Disable D backpropgation, we only train G
        for param in self.model_D_match_l2.parameters():
            param.requires_grad = False

        for param in self.model_D_match_l1.parameters():
            param.requires_grad = False

        self.optimizer_G.zero_grad()

        # get predict output
        _, _, l2, l1 = self.model(images)
        d_out_fake_l2, _ = self.model_D_match_l2(l2, label=None)
        d_out_fake_l1, _ = self.model_D_match_l1(l1, label=None)
        self.fake_l2 = l2.detach()
        self.fake_l1 = l1.detach()
        # compute loss function
        # wants to fool discriminator
        # adv_loss = self._compute_adv_loss_real(d_out_fake, loss_opt=self.adv_loss_opt)
        # adv_loss = self.loss_hinge_gen(d_out_fake)
        loss_adv_l2 = self.loss_hinge_gen(d_out_fake_l2)
    ...
    def match_dis_update(self):
        ...
        d_out_real_l2, _ = self.model_D_match_l2(self.source_l2, label=None)
        d_out_real_l1, _ = self.model_D_match_l1(self.source_l1, label=None)

        d_out_fake_l2, _ = self.model_D_match_l2(self.fake_l2, label=None)
        d_out_fake_l1, _ = self.model_D_match_l1(self.fake_l1, label=None)

        # loss = loss_real + loss_fake
        # loss = self.loss_hinge_dis(d_out_fake, d_out_real)
        loss_l1 = self.loss_hinge_dis(d_out_fake_l1, d_out_real_l1)
        loss_l2 = self.loss_hinge_dis(d_out_fake_l2, d_out_real_l2)
        ...
        # loss = loss + loss_attn
        self.loss_d_match_l1_value += loss_l1.data.cpu().numpy()
        self.loss_d_match_l2_value += loss_l2.data.cpu().numpy()
        # update loss
        self.optimizer_D_l1.step()
        self.optimizer_D_l2.step()
